...
public class LongbowItem extends BowItem {
    ...
    private static boolean hasInfinity(ItemStack bowStack, World world) {
        RegistryKey<Enchantment> infinityKey = RegistryKey.of(
                RegistryKeys.ENCHANTMENT,
                Identifier.ofVanilla("infinity")
        );

        Optional<RegistryEntry.Reference<Enchantment>> infinityOpt =
                world.getRegistryManager()
                        .get(RegistryKeys.ENCHANTMENT)
                        .getEntry(infinityKey);

        if (infinityOpt.isEmpty()) {
            return false;
        }

        return EnchantmentHelper.getLevel(infinityOpt.get(), bowStack) > 0;
    }
    ...
    public void onStoppedUsing(
        ...
        boolean infinityCoversThisShot =
                hasInfinity && (ammo.isEmpty() || isNormalArrow(ammo));

        boolean canShootWithoutAmmo =
                player.getAbilities().creativeMode || infinityCoversThisShot;
        ...
        boolean infinityFreeNormalArrow =
                hasInfinity && isNormalArrow(ammo);

        if (!world.isClient()) {
            PersistentProjectileEntity projectile =
                    arrowItem.createArrow(world, ammo, player, stack);
            ...
            if (!player.getAbilities().creativeMode) {
                projectile.pickupType = infinityFreeNormalArrow
        ...
        world.playSound(
                ...
                1.0f,
                1.0f / (
                        world.getRandom().nextFloat() * 0.4f + 1.2f
                ) + pull * 0.5f
        );
