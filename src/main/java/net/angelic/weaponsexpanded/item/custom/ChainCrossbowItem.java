...
public class ChainCrossbowItem extends CrossbowItem {
    ...
    private static void weaponsexpanded$updateLoadedVisual(ItemStack stack) {
        ...
        if (hasExplosive) {
            stack.set(
                    ...
                    new CustomModelDataComponent(WEAPONSEXPANDED$CMD_EXPLOSIVE_LOADED)
            );
        } else {
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!isChargedNow) {
            ...
            NbtCompound saved;
            if (hasSavedChamber) {
                if(root.getCompound(WEAPONSEXPANDED$SAVED_CHAMBER_KEY) != null) {
                    saved = root.getCompound(WEAPONSEXPANDED$SAVED_CHAMBER_KEY);
                } else {
                    saved = null;
                }
        ...
        if (isChargedNow) {
            ...
            if (vanillaResult == ActionResult.FAIL) {
                ...
                NbtCompound saved = restoreRoot.getCompound(WEAPONSEXPANDED$SAVED_CHAMBER_KEY);
    ...
    public boolean onStoppedUsing(ItemStack crossbow, World world, LivingEntity user, int remainingUseTicks) {
        super.onStoppedUsing(crossbow, world, user, remainingUseTicks);
        ...
        boolean toppingUp = root.contains(WEAPONSEXPANDED$SAVED_CHAMBER_KEY, NbtElement.COMPOUND_TYPE);

        // In 1.21 the superclass returns void, so inspect the resulting
        // charged state to determine whether loading succeeded.
        if (CrossbowItem.isCharged(crossbow) && toppingUp) {
            weaponsexpanded$appendCurrentChamberToQueue(world, crossbow, root);
        }

        if (toppingUp) {
            NbtCompound saved = root.getCompound(WEAPONSEXPANDED$SAVED_CHAMBER_KEY);
    ...
    public static List<ItemStack> weaponsexpanded$popNextChamber(World world, ItemStack crossbow) {
        NbtCompound root = weaponsexpanded$getOrCreateCustomNbt(crossbow);
        NbtList queue = root.getList(WEAPONSEXPANDED$QUEUE_KEY, NbtElement.COMPOUND_TYPE);
        if (queue.isEmpty()) return List.of();

        NbtCompound chamber = queue.getCompound(0);
        queue.removeFirst();
    ...
    private static List<ItemStack> weaponsexpanded$decodeChamber(World world, NbtCompound chamber) {
        ...
        NbtList list = chamber.getList(
                "projectiles",
                NbtElement.COMPOUND_TYPE
        );

        for (NbtElement nbtElement : list) {
            if (!(nbtElement instanceof NbtCompound stackTag)) {
                continue;
            }
